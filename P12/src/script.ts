...
function encontrarMayorVenta(mes: number = 0, dia: number = 0, max: { valor: number, mes: number, dia: number } = { valor: -Infinity, mes: 0, dia: 0 }): { valor: number, mes: number, dia: number } {
    if (mes === ventas.length) return max;
...
function encontrarMenorVenta(mes: number = 0, dia: number = 0, min: { valor: number, mes: number, dia: number } = { valor: Infinity, mes: 0, dia: 0 }): { valor: number, mes: number, dia: number } {
    if (mes === ventas.length) return min;
...
function calcularVentaTotal(mes: number = 0, dia: number = 0, total: number = 0): number {
    if (mes === ventas.length) return total;
...
function ventasPorDia(dia: number, mes: number = 0, suma: number = 0): number {
    if (mes === ventas.length) return suma;
...
function mostrarResultados() {
    ...
    const tabla = document.getElementById('ventas-table') as HTMLTableElement;
    tabla.innerHTML = '';  
    ventas.forEach((fila) => {
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('calcular-btn')?.addEventListener('click', mostrarResultados);
});
