...
function generarMatrizRecursiva(filas: number, columnas: number, i: number = 0, j: number = 0, matriz: number[][] = []): void {
    const matrizContainer = document.getElementById('matriz-container')!;
    
    if (i === 0 && j === 0) {
        matrizContainer.innerHTML = ''; // Limpiar el contenedor
...
function calcularResultados(matriz: number[][], i: number, j: number, sumaFilas: number[], promedioFilas: string[], sumaColumnas: number[], promedioColumnas: string[]): void {
...
function mostrarResultados(sumaFilas: number[], promedioFilas: string[], sumaColumnas: number[], promedioColumnas: string[], fila: number, columna: number): void {
    if (fila >= sumaFilas.length && columna >= sumaColumnas.length) return;
